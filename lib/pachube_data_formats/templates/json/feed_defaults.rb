      ...
      module FeedDefaults
        ...
        def location_hash
          { :disposition => location_disposition,
            ...
            :ele => location_ele,
            :lat => location_lat,
            :lon => location_lon }.delete_if_nil_value if location_disposition || location_name || location_exposure || location_domain || location_ele || location_lat || location_lon
        end

        def unit_hash(datastream)
          { :type => datastream.unit_type,
            :symbol => datastream.unit_symbol,
            :label => datastream.unit_label }.delete_if_nil_value if datastream.unit_type || datastream.unit_label || datastream.unit_symbol
        end
      end
    end
  end
