...
describe "default feed json templates" do
  ...
  context "2 (used by API v2)" do
    ...
    it "should represent Pachube JSON with datapoints" do
      ...
      expected_csv = @feed.datastreams.collect do |datastream|
        if datastream.datapoints.any?
          datastream.datapoints.collect {|dp| CSV.generate_line([datastream.id, dp.at.iso8601(6), dp.value]) }
        else
          CSV.generate_line([datastream.id, datastream.updated.iso8601(6), datastream.current_value])
        end
      end.join("\n")
    ...
    it "should allow representing full Pachube JSON with datapoints" do
      ...
      expected_csv = @feed.datastreams.collect do |datastream|
        if datastream.datapoints.any?
          datastream.datapoints.collect {|dp| CSV.generate_line([@feed.id, datastream.id, dp.at.iso8601(6), dp.value]) }
        else
          CSV.generate_line([@feed.id, datastream.id, datastream.updated.iso8601(6), datastream.current_value])
        end
      end.join("\n")
